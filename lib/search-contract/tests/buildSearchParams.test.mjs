...
test("car engines include journey chips plus facet-gated fuel/transmission", async () => {
  const { enginesForCategory, engineByKey } = await import("../src/engines.ts");
  ...
  assert.equal(engineByKey("car", "petrol")?.requiresFacet, true);
  assert.equal(engineByKey("car", "automatic")?.requiresFacet, true);
  assert.equal(engineByKey("car", "import")?.requiresFacet, undefined);
  ...
  const fuelEngine = buildSearchParams({
    ...DEFAULT_CRITERIA,
    category: "car",
    engineKey: "diesel",
  });
  assert.equal(fuelEngine.fuel_type, "diesel");
