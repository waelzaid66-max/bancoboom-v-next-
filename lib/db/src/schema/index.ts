...
export const messages = pgTable(
  "messages",
  {
    ...
    senderId: uuid("sender_id")
      .references(() => users.id, { onDelete: "cascade" })
      .notNull(),
    // Stable UUID generated by the sending client. Existing rows remain null;
    // new clients send it so retries can return the original durable message
    // instead of incrementing unread counters or notifications twice.
  ...
  (table) => [
    ...
    uniqueIndex("uniq_message_client_attempt").on(
