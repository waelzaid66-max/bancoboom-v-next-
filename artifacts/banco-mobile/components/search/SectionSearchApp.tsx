...
export function SectionSearchApp({
  ...
  const reHeaderTypeTabs = useMemo(() => {
    ...
    return [
      ...
      { value: RE_MORE_TAB, label: t("search.discover.section.deskMore") },
    ];
  ...
  const reActiveChips = useMemo(() => {
    ...
    if (criteria.location) {
      chips.push({ id: "location", label: criteria.location, onClear: () => update({ location: "" }) });
    }
    if (criteria.nearMeEnabled) {
      chips.push({
        id: "nearMe",
      ...
      <View style={styles.emptyState}>
        ...
        {activeGroup ? (
          ...
          >
            ...
            <AppText style={[styles.emptyCtaText, { color: colors.foreground }]}>
              {t("search.emptyRfq")}
            </AppText>
          </Pressable>
        ) : null}
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      {isRealEstateSection ? (
        ...
        >
          <Pressable onPress={goBack} style={styles.backBtn} hitSlop={12} testID="section-back">
          ...
          <View style={styles.headerTitleWrap}>
            <View style={[styles.headerTitleRow, { flexDirection: rowDir }]}>
              ...
              <AppText style={[styles.headerTitle, { color: colors.foreground, textAlign }]} numberOfLines={1}>
                {t(titleKey)}
              </AppText>
            </View>
            {subtitleKey ? (
              <AppText style={[styles.headerSub, { color: colors.mutedForeground, textAlign }]} numberOfLines={1}>
                {t(subtitleKey)}
              </AppText>
            ) : null}
          </View>
          <Pressable
            onPress={openSearch}
            style={[styles.iconBtn, { backgroundColor: draftQuery ? accent : colors.secondary, borderRadius: colors.radius }]}
          ...
          <Pressable
            ...
            style={[styles.iconBtn, { backgroundColor: activeFilterCount > 0 ? accent : colors.secondary, borderRadius: colors.radius }]}
            testID="section-filter-toggle"
          >
            <Feather name="sliders" size={18} color={activeFilterCount > 0 ? "#FFFFFF" : colors.foreground} />
            {activeFilterCount > 0 ? (
              <View style={[styles.filterBadge, { backgroundColor: "#FFFFFF" }]}>
                <AppText style={[styles.filterBadgeText, { color: accent }]}>{activeFilterCount}</AppText>
              </View>
            ) : null}
          </Pressable>
        </View>
      ...
      {!isRealEstateSection && !isMaterialsSection && !isCarSection && searchOpen ? (
        <View style={[styles.searchBar, { flexDirection: rowDir, backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
          ...
          <TextInput
            ...
            autoCorrect={false}
          />
          <Pressable onPress={handleSaveSearch} disabled={searchSaved} hitSlop={8} testID="section-save-search">
            <Feather name="bookmark" size={16} color={searchSaved ? accent : colors.mutedForeground} />
          </Pressable>
        ...
        <View style={[styles.suggestions, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
          {suggestions.map((s, i) => (
            <Pressable
              ...
              style={[styles.suggestionItem, { flexDirection: rowDir, borderBottomColor: i < suggestions.length - 1 ? colors.border : "transparent" }]}
            >
              <Ionicons name="search-outline" size={14} color={colors.mutedForeground} />
              <AppText style={[styles.suggestionText, { color: colors.foreground, textAlign }]}>{s}</AppText>
            </Pressable>
          ))}
        </View>
      ) : null}
          ...
          >
            ...
            <Pressable
              onPress={() => {
                playSound(tap);
                const cycle = ["recommended", "newest", "price_asc", "price_desc"] as const;
                const next = cycle[(cycle.indexOf(criteria.sort as (typeof cycle)[number]) + 1) % cycle.length];
                update({ sort: next });
              }}
              style={[styles.sortChip, { backgroundColor: criteria.sort !== "recommended" ? accent : colors.secondary, flexDirection: rowDir }]}
              accessibilityLabel={t(`search.sortOptions.${criteria.sort}`)}
              testID="section-sort-cycle"
            >
              <Feather
                name={criteria.sort === "price_asc" ? "trending-up" : criteria.sort === "price_desc" ? "trending-down" : criteria.sort === "newest" ? "clock" : "list"}
                size={14}
                color={criteria.sort !== "recommended" ? "#FFFFFF" : colors.mutedForeground}
              />
            </Pressable>
            {showListingMode ? <View style={[styles.chipStripDivider, { backgroundColor: colors.border }]} /> : null}
            {showListingMode ? (
              ...
              {showEngineChips ? stripEngineList.map((e) => {
                ...
                return (
                  <Pressable
                    ...
                    testID={`engine-${e.key}`}
                  >
                    <AppText style={[styles.stripChipText, { color: active ? "#FFFFFF" : colors.mutedForeground }]}>{t(e.i18nKey)}</AppText>
                  </Pressable>
                );
              }) : null}
              {showIndustrialChips ? [
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => {
                      playSound(tap);
                      Haptics.selectionAsync();
                      selectIndustrialType(item.key);
                    }}
                    style={[styles.stripChip, { backgroundColor: active ? accent : colors.secondary }]}
                    testID={`industrial-type-${item.key}`}
                  >
                    <AppText style={[styles.stripChipText, { color: active ? "#FFFFFF" : colors.mutedForeground }]}>{t(item.i18nKey)}</AppText>
                  </Pressable>
                );
              }) : null}
            </ScrollView>
      ...
      {showReTypeStrip ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.hScroll} contentContainerStyle={[styles.reTypeStrip, { flexDirection: rowDir }]} testID="re-type-strip">
          {[{ value: RE_TYPE_ALL, label: t("home.engines.all") }]
            .concat(reTypeTabs.map((v) => {
              const def = PROPERTY_TYPES.find((p) => p.value === v);
              return { value: v, label: def ? (isRTL ? def.ar : def.en) : v };
            }))
            .map((tab) => {
              const active = tab.value === RE_TYPE_ALL ? !criteria.propertyType : criteria.propertyType === tab.value;
              return (
                <Pressable
                  key={tab.value}
                  onPress={() => {
                  ...
                  style={[styles.stripChip, { backgroundColor: active ? accent : colors.card, borderWidth: 1, borderColor: active ? accent : colors.border }]}
                  testID={`re-type-${tab.value}`}
                >
                  <AppText style={[styles.stripChipText, { color: active ? "#FFFFFF" : colors.foreground }]}>{tab.label}</AppText>
                </Pressable>
              );
            })}
        </ScrollView>
      ...
      {showCarBrandStrip && !isCarSection ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={[styles.hScroll, styles.carFilterPanelFooter]} contentContainerStyle={[styles.chipStrip, { flexDirection: rowDir }]} testID="car-brand-origin-strip" />
      ) : null}
      ...
      {showMaterialsAxisStrip ? (
        <ScrollView
          ...
          contentContainerStyle={[styles.materialsAxisStrip, { flexDirection: rowDir }]}
          ...
          {materialsAxisTabs.map((tab) => {
            ...
            return (
              <Pressable
                ...
                style={[styles.materialsAxisChip, { backgroundColor: active ? accent : colors.secondary, borderColor: active ? accent : colors.border }]}
                testID={`industrial-type-${tab.value}`}
              >
                <AppText style={[styles.materialsAxisChipText, { color: active ? "#FFFFFF" : colors.mutedForeground }]} numberOfLines={1}>{tab.label}</AppText>
              </Pressable>
            );
          })}
          <View style={[styles.materialsAxisDivider, { backgroundColor: colors.border }]} />
          <View style={[styles.materialsOriginCluster, { flexDirection: rowDir }]} testID="materials-origin-strip">
            {(["all", "local", "imported"] as const).map((o) => {
              ...
              return (
                ...
                >
                  <AppText style={[styles.materialsAxisChipText, { color: active ? "#FFFFFF" : colors.mutedForeground }]} numberOfLines={1}>
      ...
      {showMaterialsLayer2 ? (
        <ScrollView
          ...
          showsHorizontalScrollIndicator={false}
          style={styles.hScroll}
          contentContainerStyle={[styles.reTypeStrip, styles.materialsLayer2Strip, { flexDirection: rowDir }]}
        ...
        >
          <Pressable
            ...
            style={[styles.stripChip, styles.materialsCommodityChip, { backgroundColor: !criteria.material ? accent : colors.card, borderWidth: 1, borderColor: !criteria.material ? accent : colors.border }]}
            testID="materials-material-all"
          >
            <AppText style={[styles.stripChipText, { color: !criteria.material ? "#FFFFFF" : colors.foreground }]}>{t("home.engines.all")}</AppText>
          </Pressable>
          {MATERIAL_TYPES.map((m) => {
            ...
            return (
              <Pressable
                ...
                style={[styles.stripChip, styles.materialsCommodityChip, { backgroundColor: active ? accent : colors.card, borderWidth: 1, borderColor: active ? accent : colors.border }]}
                testID={`materials-material-${m.value}`}
              >
                <AppText style={[styles.stripChipText, { color: active ? "#FFFFFF" : colors.foreground }]}>{isRTL ? m.ar : m.en}</AppText>
              </Pressable>
            );
          })}
        </ScrollView>
      ...
      {showRentalTerms ? (
        <View style={[styles.rentalChrome, { flexDirection: rowDir }]} testID="section-rental-chrome">
          <FilterPillSelect
            icon="calendar"
            title={t("create.fields.rentalTerm")}
            options={rentalTerms.map((r) => ({ value: r.value, label: isRTL ? r.ar : r.en }))}
      ...
      {isRealEstateSection && reActiveChips.length > 0 ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.hScroll} contentContainerStyle={[styles.activeChipStrip, { flexDirection: rowDir }]} testID="re-active-filters">
          {reActiveChips.map((chip) => (
            <Pressable
              ...
              style={[styles.activeChip, { backgroundColor: `${accent}22`, borderColor: accent, flexDirection: rowDir }]}
              testID={`re-active-${chip.id}`}
            >
              <AppText style={[styles.activeChipText, { color: accent }]} numberOfLines={1}>{chip.label}</AppText>
      ...
      {viewState === "results" && items.length > 0 && !(isCarSection && mapMode) ? (
        <AppText style={[styles.resultsCount, { color: colors.mutedForeground, textAlign }]} testID="section-results-count">
          {t("search.resultsCount", { count: `${items.length}${hasNext ? "+" : ""}` })}
        </AppText>
      ) : null}
      ...
      <FilterSheet
        ...
        propertyTypeOptions={isRealEstateSection ? [...RE_TYPE_PRIMARY] : undefined}
        onSelectCategory={() => {}}
        onSelectEngine={selectEngine}
        onBrowseBrand={browseBrandChip}
        onOpenBrandPicker={() => setCarPickerOpen(true)}
        onUpdate={(partial) => {
          if (partial.marketCountry) void savePreferredMarketCountry(partial.marketCountry);
          if (partial.rentalTerm && criteria.category === "real_estate" && criteria.engineKey !== "rent") {
      ...
      <View style={styles.resultsArea}>
        ...
        {mapMode && inResultsView ? (
          <SearchResultsMap
            ...
            onOpenListingId={(id) => {
              ...
              router.push(focusBooking ? `/listing/${id}?focus=booking` : `/listing/${id}`);
            }}
            onSave={toggleSave}
            isSaved={isSaved}
          />
        ...
        {showMapChrome && !isCarSection ? (
          <View style={[styles.mapToggleWrap, { bottom: insets.bottom + 88 }]} pointerEvents="box-none">
            <Pressable
              onPress={() => {
                playSound(tap);
              ...
              style={[styles.mapToggle, { backgroundColor: colors.foreground, flexDirection: isRTL ? "row-reverse" : "row" }]}
              testID="section-map-toggle"
            >
              <Feather name={mapMode ? "list" : "map"} size={16} color={colors.background} />
              <AppText style={[styles.mapToggleText, { color: colors.background }]}>
                {mapMode ? t("search.viewList") : hasPagePins ? `${t("search.viewMap")} (${mappableItems.length})` : t("search.viewMap")}
              </AppText>
            </Pressable>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  headerTitleRow: { alignItems: "center", gap: 8, minWidth: 0 },
  headerIcon: { width: 26, height: 26, borderRadius: 8, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  headerTitle: { flex: 1, minWidth: 0, fontSize: 18, fontFamily: "Inter_700Bold" },
  headerBrand: { fontSize: 10, fontFamily: "Inter_700Bold", letterSpacing: 0.6, marginTop: 2 },
  headerSub: { fontSize: 12, fontFamily: "Inter_400Regular", marginTop: 2 },
  hScroll: { flexGrow: 0 },
  searchBar: { alignItems: "center", gap: 10, marginHorizontal: 16, marginTop: 10, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderRadius: 12 },
  searchBarInput: { flex: 1, fontSize: 15, fontFamily: "Inter_400Regular", padding: 0 },
  iconBtn: { padding: 12, position: "relative", flexShrink: 0 },
  filterBadge: { position: "absolute", top: 6, end: 6, width: 14, height: 14, borderRadius: 7, alignItems: "center", justifyContent: "center" },
  filterBadgeText: { fontSize: 9, fontFamily: "Inter_700Bold" },
  chipRow: { gap: 8, paddingHorizontal: 12, paddingTop: 8, paddingBottom: 2 },
  chip: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 18 },
  chipText: { fontSize: 13, fontFamily: "Inter_500Medium" },
  chipStrip: { alignItems: "center", flexWrap: "wrap", flexGrow: 0, gap: 6, paddingHorizontal: 12, paddingTop: 6, paddingBottom: 2 },
  chipStripRow: { alignItems: "center", flexGrow: 0, gap: 6, paddingHorizontal: 12, paddingTop: 6, paddingBottom: 2 },
  carFilterPanel: { backgroundColor: SECTION_NEUTRAL.void, paddingTop: 10 },
  carFilterPanelFooter: { backgroundColor: SECTION_NEUTRAL.void, borderBottomLeftRadius: 20, borderBottomRightRadius: 20, paddingBottom: 10, shadowColor: "#000000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.18, shadowRadius: 8, elevation: 6, zIndex: 10 },
  chipStripDivider: { width: 1, height: 20, opacity: 0.5, marginHorizontal: 2 },
  carFilterToggle: { alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
  carFilterToggleText: { fontSize: 13, fontFamily: "Inter_600SemiBold" },
  carFilterToggleCount: { minWidth: 18, height: 18, borderRadius: 9, alignItems: "center", justifyContent: "center", paddingHorizontal: 5 },
  carFilterToggleCountText: { color: "#FFFFFF", fontSize: 10, fontFamily: "Inter_700Bold" },
  stripChip: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 16 },
  materialsAxisStrip: { alignItems: "center", gap: 6, paddingHorizontal: 12, paddingTop: 4, paddingBottom: 1 },
  materialsAxisChip: { paddingHorizontal: 11, paddingVertical: 5, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth },
  materialsAxisChipText: { fontSize: 12, fontFamily: "Inter_600SemiBold" },
  materialsAxisDivider: { width: StyleSheet.hairlineWidth, height: 18, opacity: 0.7, marginHorizontal: 2 },
  materialsOriginCluster: { alignItems: "center", gap: 6 },
  materialsLayer2Strip: { alignItems: "center", gap: 6, paddingTop: 3, paddingBottom: 1 },
  materialsCommodityChip: { paddingHorizontal: 12, paddingVertical: 5 },
  carBrandBtn: { alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 18, borderWidth: StyleSheet.hairlineWidth, maxWidth: 150, flexShrink: 1 },
  sortChip: { alignItems: "center", justifyContent: "center", width: 34, height: 34, borderRadius: 17 },
  stripChipText: { fontSize: 12.5, fontFamily: "Inter_500Medium" },
  rentalChrome: { alignItems: "center", gap: 8, paddingHorizontal: 12, paddingTop: 8, paddingBottom: 2 },
  reTypeStrip: { alignItems: "center", gap: 8, paddingHorizontal: 12, paddingTop: 8, paddingBottom: 2 },
  resultsCount: { fontSize: 12.5, paddingHorizontal: 16, paddingTop: 8 },
  activeChipStrip: { alignItems: "center", gap: 8, paddingHorizontal: 12, paddingTop: 6, paddingBottom: 2 },
  activeChip: { alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16, borderWidth: 1, maxWidth: 180 },
  activeChipText: { fontSize: 12, fontFamily: "Inter_600SemiBold", flexShrink: 1 },
  suggestions: { marginHorizontal: 16, marginTop: 2, borderWidth: 1, overflow: "hidden" },
  suggestionItem: { alignItems: "center", gap: 10, paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: 1 },
  suggestionText: { fontSize: 14, fontFamily: "Inter_400Regular" },
  mapToggleWrap: { position: "absolute", left: 0, right: 0, alignItems: "center" },
  mapToggle: { alignItems: "center", gap: 7, paddingVertical: 10, paddingHorizontal: 18, borderRadius: 999, elevation: 6, shadowColor: "#000000", shadowOpacity: 0.2, shadowRadius: 8, shadowOffset: { width: 0, height: 3 } },
  mapToggleText: { fontSize: 14, fontWeight: "700" },
  applyBtn: { marginTop: 14, paddingVertical: 12, alignItems: "center" },
  ...
  emptyCta: { alignItems: "center", gap: 8, paddingVertical: 11, paddingHorizontal: 20, marginTop: 6 },
