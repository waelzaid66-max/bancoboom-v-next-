...
| P0 frozen slice | Section-specific integration architecture | `SectionSearchApp.tsx` conflicted merge lineage `a61c1e1`, `11d8185`; current blob `bd0f46e`; VNX-05F `be172d1`, CI `31451674276` | Historical source remains `CONFLICT_DAMAGED`; bounded four-catalogue composition/state/map contracts `TESTED`; never restore wholesale | VNX-05G must mount independent `BookingStaysApp`; live loading/results/empty/error/facets/pagination/scroll/map, 320–430, accessibility, provider and device matrices remain |
