...
| P0 frozen | Messenger durable body-text outbox | Owner-attributed advanced wave has no recovered object; base thread blob `62412e8` was component-memory only; VNX-07A `5c2631a`, tree `ebf2c81`, CI `31460794057` | `RECOVER` as bounded reconstruction; account-bound persist-before-POST path `MODERNIZED/TESTED`; unchanged same-UUID server transaction PostgreSQL-verified | Android/iOS airplane-mode, kill/relaunch, two-account switching, live Clerk/network, encrypted-at-rest and PII-free telemetry remain `UNPROVEN`; replies/offers/media excluded |
...
| P0 active | Maps runtime | `127e3d7`, `a4c1eb0`, `34709b4`, `12ce4f4`; VNX-06A `0214983`, CI `31454274073`; VNX-06B `0341b65`, CI `31455520472`; VNX-06C base/final hosts `e8c9c65`/`ebd9db4` → `289acb6`/`cb54dc3`, CI `31457288589` | Draw-area, hub-world hydration, and stale criteria-response slices are now frozen as bounded recovery/modernization. Remaining Maps claims are runtime `UNPROVEN`, not assumed missing | Real browser/WebView/provider/Android/iOS, large-result/latency and rapid churn, map/list/five-domain, MapPinPicker/accessibility/device certification |
| P0 frozen recovery candidate | Messenger unread/read projection serialization | Reproduced e4 interleaving between transactional send and two autocommit mark-read updates; VNX-07B product/test `2892179`, runtime decoder follow-up `2e659bb`; CI `31706332675` | `RECOVER/MODERNIZE/RUNTIME_VERIFY` completed at PostgreSQL scope on the published recovery branch; canonical promotion remains separate | Physical-device/live multi-replica stress remains `UNPROVEN`; block/mute/realtime and non-text durability are not covered |
| P0 later | Messenger block/mute and durable non-text sends | Owner-attributed advanced wave, no recovered object; VNX-07A covers body text and VNX-07B covers bounded read/unread serialization only | `UNPROVEN`; separate policy/contract batches, never infer closure from either accepted slice | Product policies, schema/API design, private-media invariants, DB/device tests |
...
| P0 frozen control | Migration operator authority | stale compose/operator blobs `26bbcf3`, `42a9a4f`, `479f6d7`, `6d1b6ac`, `e9c0bf3`; correction `e4b8f297`; confidence guard blob `11154d4`; CI `31462992521` | VNX-OPS-02 `TESTED`: seven operator surfaces now use committed migrations and Postgres → migrate → API without changing runtime commands, migrations, workflows, or deployment | Manual CI 7/7 is source/static/PostgreSQL evidence only. Production DB equivalence/adoption, Docker/Compose/Coolify, backup/restore, deploy and final-RC workspace lint remain separate |
| P0 active control | Final-RC lint coverage | VNX-LINT-01 declaration policy; Dealer defect separated into `8396b39`; VNX-LINT-02 cleanup `875406e3` | Scripts/API/DB and Website/Landing are 0-error/0-warning; full monorepo remains `UNPROVEN` because Admin/Dealer/Mobile/Sandbox have no lint scripts | Install/configure the React Hooks rule, define React Native asset and Node/tooling scopes, exclude vendored code narrowly, wire all four workspaces, then adjudicate 49 source files (51 errors/61 warnings) without blanket ignores |
