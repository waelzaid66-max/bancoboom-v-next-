...
| P0 later | Messenger read cursor/block/mute and durable non-text sends | Owner-attributed advanced wave, no recovered object; VNX-07A covers body text only | `UNPROVEN`; separate policy/contract batches, never infer closure from the text outbox | Product policies, schema/API design, private-media invariants, DB/device tests |
...
| P0 next ops | Migration operator authority and lint coverage | executable migrate authority is preserved, but operator docs at blobs `26bbcf3`, `42a9a4f`, `479f6d7`, `6d1b6ac`, `e9c0bf3` retain obsolete push-force wording; root lint blob `17cc291` covers scripts only | `MUTATED`; close as VNX-OPS-02 docs/static-guard batch without changing compose runtime or migrations; design full workspace lint separately | RED on exact contradictory surfaces, corrected committed-migration order, confidence guard, DB check/chain/root build; final-RC workspace lint still required |
