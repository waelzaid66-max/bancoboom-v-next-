...
| P0 | Section-specific integration architecture | `SectionSearchApp.tsx` conflicted merge lineage `a61c1e1`, `11d8185`; current blob `bd0f46e` | `CONFLICT_DAMAGED/UNPROVEN`; adjudicate per capability, never wholesale | Standalone Cars/Property/Facilities headers are frozen; Materials and each combined capability matrix remain; Stay remains independent app |
...
| P0 frozen | Stay identity/header usability | `80b1a17` split → `fdbb4ff` revert → `e66a561` rebuild → hidden-overlay/collapse fix `d098047`; current blobs `47e583d`/`42bdfb8`; target `e85cd39`; CI `31406559372` | `ALREADY_PRESERVED`; renderer protection added with no product delta | Complete booking journey, 320–430 AR/EN/RTL/LTR, accessibility, Android/iOS/device runtime |
...
| P0 | Materials identity/header usability | split/collapse `1bfa485` → shared-neutral migration `e495e02`; current blob `b088456` | `RECOVER` only after independent renderer and merge adjudication | Empty/error/scrolling/pinned state proof; current 320–430 and device matrix |
