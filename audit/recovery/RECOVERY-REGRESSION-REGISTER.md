...
| Ambiguous Messenger retry duplicated message/unread | target reconstruction `e318cef` | DB schema/migration `0006`, API, mobile thread | Scoped unique UUID, atomic message/unread transaction, API/mobile tests; PostgreSQL/device `UNPROVEN` |
| Message commit could lose its notification work on process stop | no recovered advanced object; billing precedent `ae52fe3` | Message notification outbox (VNX-03 in flight) | Must atomically enqueue, retry with dedupe, expose readiness, and pass DB/runtime gates before freeze |
...
| Schema authority bypass through push-force/manual edits | migration line through `0005`, then target `0006` | `lib/db`, migration scripts, CI/deploy | Generated additive migrations, drift check, migrate×2 PostgreSQL gate; never force-push schema |
